...
router.get("/pokedex/:pokeIndex", async (req, res, next) => {
  ...
  try {
   ...
   for(let i = 0; i< pokemonDetails.data.stats.length; i++) {
     pokemonDetails.data.stats[i].stat.name = capitalize(pokemonDetails.data.stats[i].stat.name)
    }
    ...
    let pokemonSpecies = await axios.get(
      ...
      pokemonSpecies.data.habitat.name = capitalize(pokemonSpecies.data.habitat.name)
    let pokemonMinus;
